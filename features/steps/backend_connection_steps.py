...
def step_impl_check_backend_connection(context):
    ...
    if hasattr(context, 'use_mock_mcp') and context.use_mock_mcp:
        logger.info("Using mock MCP server for testing")
    else:
        assert os.environ.get('SHOPIFY_MCP_SERVER') or hasattr(context, 'shopify_mcp_server'), \
            "SHOPIFY_MCP_SERVER environment variable or context.shopify_mcp_server not set"
    
    # Log the MCP server URL being used
    mcp_server = os.environ.get('SHOPIFY_MCP_SERVER', getattr(context, 'shopify_mcp_server', 'mock'))
...
def step_impl_query_products(context):
    ...
    assert hasattr(context, 'mcp_connection_successful'), "Backend connection not established"
